...
router.post('/cart/add', verifyTokenExceptLogin, async (req, res) => {
    ...
    let existingItem = req.session.cart.find(item => item.service_id === service.id);
...
router.post('/cart/checkout', verifyTokenExceptLogin, async (req, res) => {
    ...
    const totalPrice = req.session.cart.reduce((sum, item) => sum + (item.price * item.quantity), 0);
